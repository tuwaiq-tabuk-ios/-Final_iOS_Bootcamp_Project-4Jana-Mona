...
extension UIViewController {
  ...
  func showAlertPopUp(title: String,
                      message: String,
                      buttonTitle1: String = "Ok".localize_,
                      buttonTitle2: String = "Cancel".localize_,
                      buttonTitle1Style: UIAlertAction.Style = .default,
                      buttonTitle2Style: UIAlertAction.Style = .default, action1
                      buttonTitle1Action: @escaping (() -> Void), action2
                      buttonTitle2Action: @escaping (()->Void)) {
    
    let alert = UIAlertController.init(title: title,
                                       message: message,
                                       preferredStyle: .alert)
    
    let button1 = UIAlertAction.init(title: buttonTitle1,
                                     style: buttonTitle1Style) { (action) in
      print("\(buttonTitle1) Button")
      buttonTitle1Action()
    }
    let button2 = UIAlertAction.init(title: buttonTitle2,
                                     style: buttonTitle2Style) { (action) in
  ...
  func showAlertError(title: String = "Error".localize_,
                      message: String) {
  ...
  func showSnackbar(message: String,
                    buttonTitle1: String = "Ok".localize_,
                    buttonTitle2: String = "Close".localize_,
                    showButtonOne: Bool = false,
                    showButtonTwo: Bool = false,
                    isError: Bool = false,
                    action1 buttonOneAction: @escaping (() -> Void),
                    buttonTwoAction: @escaping (() -> Void),
                    backgroundColor: String = "#008800") {
